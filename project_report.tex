...
\begin{algorithm}[H]
\caption{Power Flow Agent - Multi-Tool Orchestration}
\begin{algorithmic}[1]
\STATE \textbf{Input:} User query $q$
\STATE \textbf{Output:} Synthesized markdown response $R$

\STATE Initialize message history: $M \leftarrow [system\_prompt, q]$
\STATE Define tools: $T = \{$Ybus, GaussSeidel, Loss, Fault$\}$
\STATE iteration $\leftarrow 0$, max\_iter $\leftarrow 10$

\WHILE{iteration $<$ max\_iter}
    \STATE Call Groq LLM: response $\leftarrow$ \texttt{chat.create}($M$, tools=$T$)
    \STATE Extract tool\_calls from response
    
    \IF{tool\_calls = $\emptyset$}
        \STATE \textbf{break} // No more tools needed, final answer ready
    \ENDIF
    
    \STATE Append response to $M$
    
    \FOR{each tool\_call in tool\_calls}
        \STATE Parse: $(tool_{name}, args) \leftarrow$ tool\_call
        
        \IF{$tool_{name} = $ ``run\_ybus\_calculation\_agent''}
            \STATE $result \leftarrow$ \texttt{YbusAgent}($args.query$)
        \ELSIF{$tool_{name} = $ ``run\_power\_flow\_agent''}
            \STATE $result \leftarrow$ \texttt{GaussSeidelAgent}($args.query$)
        \ELSIF{$tool_{name} = $ ``run\_loss\_agent''}
            \STATE $result \leftarrow$ \texttt{LossAgent}($args.query$)
        \ELSIF{$tool_{name} = $ ``run\_bolted\_fault\_agent''}
            \STATE $result \leftarrow$ \texttt{FaultAgent}($args.query$)
        \ENDIF
        
        \STATE Append $(tool_{name}, result)$ to $M$ as tool response
    \ENDFOR
    
    \STATE iteration $\leftarrow$ iteration + 1
\ENDWHILE

\STATE Extract final text response from last LLM call
\STATE \textbf{return} formatted markdown with disclaimer
\end{algorithmic}
\end{algorithm}
